...
// Project
#include "../../../../include/libmrhevdata/libmrhevdata/Version/1/MRH_EvCustom_V1.h"
...
int MRH_EVD_C_ToEvent_V1(MRH_Event* p_Event, MRH_Uint32 u32_Type, const void* p_Data)
{
    ...
    if (u32_DataSize > 0)
    {
        ...
        {
            case MRH_EVENT_CUSTOM_AVAIL_S:
                memcpy(&(p_Event->p_Data[0]), &(((MRH_EvD_C_ServiceAvail_S*)p_Data)->u8_Available), 1);
                memcpy(&(p_Event->p_Data[1]), &(((MRH_EvD_C_ServiceAvail_S*)p_Data)->u32_SupplierID), 4);
                memcpy(&(p_Event->p_Data[5]), &(((MRH_EvD_C_ServiceAvail_S*)p_Data)->u32_BinaryID), 4);
                memcpy(&(p_Event->p_Data[9]), &(((MRH_EvD_C_ServiceAvail_S*)p_Data)->u32_Version), 4);
                break;
...
int MRH_EVD_C_ToData_V1(void* p_Data, MRH_Uint32 u32_Type, const MRH_Event* p_Event)
{
    if (p_Data == NULL)
    {
        return -1;
    ...
    {
        case MRH_EVENT_CUSTOM_AVAIL_S:
            memcpy(&(((MRH_EvD_C_ServiceAvail_S*)p_Data)->u8_Available), &(p_Event->p_Data[0]), 1);
            memcpy(&(((MRH_EvD_C_ServiceAvail_S*)p_Data)->u32_SupplierID), &(p_Event->p_Data[1]), 4);
            memcpy(&(((MRH_EvD_C_ServiceAvail_S*)p_Data)->u32_BinaryID), &(p_Event->p_Data[5]), 4);
            memcpy(&(((MRH_EvD_C_ServiceAvail_S*)p_Data)->u32_Version), &(p_Event->p_Data[9]), 4);
            return 0;
