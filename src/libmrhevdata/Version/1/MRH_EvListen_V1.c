...
// Project
#include "../../../../include/libmrhevdata/libmrhevdata/Version/1/MRH_EvListen_V1.h"
...
int MRH_EVD_L_ToEvent_V1(MRH_Event* p_Event, MRH_Uint32 u32_Type, const void* p_Data)
{
    ...
    switch (u32_Type)
    {
        ...
        case MRH_EVENT_LISTEN_STRING_S:
            ...
            u32_DataSize += strnlen((((MRH_EvD_L_String_S*)p_Data)->p_String), MRH_EVD_L_STRING_BUFFER_MAX);
            break;
    ...
    if (u32_DataSize > 0)
    {
        ...
        {
            case MRH_EVENT_LISTEN_AVAIL_S:
                memcpy(&(p_Event->p_Data[0]), &(((MRH_EvD_L_ServiceAvail_S*)p_Data)->u8_Available), 1);
                memcpy(&(p_Event->p_Data[1]), &(((MRH_EvD_L_ServiceAvail_S*)p_Data)->u32_SupplierID), 4);
                memcpy(&(p_Event->p_Data[5]), &(((MRH_EvD_L_ServiceAvail_S*)p_Data)->u32_BinaryID), 4);
                memcpy(&(p_Event->p_Data[9]), &(((MRH_EvD_L_ServiceAvail_S*)p_Data)->u32_Version), 4);
                break;
                
            case MRH_EVENT_LISTEN_GET_METHOD_S:
                memcpy(&(p_Event->p_Data[0]), &(((MRH_EvD_L_GetMethod_S*)p_Data)->u8_Result), 1);
                memcpy(&(p_Event->p_Data[1]), &(((MRH_EvD_L_GetMethod_S*)p_Data)->u8_Method), 1);
                break;
                
            case MRH_EVENT_LISTEN_STRING_S:
                memcpy(&(p_Event->p_Data[0]), &(((MRH_EvD_L_String_S*)p_Data)->u32_ID), 4);
                
                if (u32_DataSize > 4)
                {
                    memcpy(&(p_Event->p_Data[4]), (((MRH_EvD_L_String_S*)p_Data)->p_String), u32_DataSize - 4);
                }
                break;
...
int MRH_EVD_L_ToData_V1(void* p_Data, MRH_Uint32 u32_Type, const MRH_Event* p_Event)
{
    if (p_Data == NULL)
    {
        return -1;
    ...
    {
        case MRH_EVENT_LISTEN_AVAIL_S:
            memcpy(&(((MRH_EvD_L_ServiceAvail_S*)p_Data)->u8_Available), &(p_Event->p_Data[0]), 1);
            memcpy(&(((MRH_EvD_L_ServiceAvail_S*)p_Data)->u32_SupplierID), &(p_Event->p_Data[1]), 4);
            memcpy(&(((MRH_EvD_L_ServiceAvail_S*)p_Data)->u32_BinaryID), &(p_Event->p_Data[5]), 4);
            memcpy(&(((MRH_EvD_L_ServiceAvail_S*)p_Data)->u32_Version), &(p_Event->p_Data[9]), 4);
            return 0;
        ...
        case MRH_EVENT_LISTEN_STRING_S:
            memcpy(&(((MRH_EvD_L_String_S*)p_Data)->u32_ID), &(p_Event->p_Data[0]), 4);
            memset((((MRH_EvD_L_String_S*)p_Data)->p_String), '\0', MRH_EVD_L_STRING_BUFFER_MAX_TERMINATED);
            
            if (p_Event->u32_DataSize > 4)
            {
                memcpy((((MRH_EvD_L_String_S*)p_Data)->p_String), &(p_Event->p_Data[4]), p_Event->u32_DataSize - 4);
            }
            return 0;
